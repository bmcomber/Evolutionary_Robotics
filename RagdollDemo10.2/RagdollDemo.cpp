...
void RagdollDemo::passWeights(int argc, char* argv[])
{
    /*if (argc != 33)
    {
    ...
    for (int i = 1; i <= 32; ++i)
    {
        weights[(int)(i/8)][i%8] = atof(argv[i]);
...
void RagdollDemo::initPhysics()
{
    ...
    //std::ifstream ifile("weights.txt");
    ...
    */
    ...
    //Im gonna get this fuckin file read thing tonight
    /*ifstream file("/Users/bmcomber/Assignment10/Assignment_10/weights.dat");
    string line;
	for(int i=0; i<4; i++){
		for(int j=0; j<8; j++){
    ...
    ifstream inFile;
    inFile.open("/Users/bmcomber/Assignment10.2/weights2.dat");
	string line;
    //if(inFile.is_open())
      //  printf("You got it\n");
    for(int j=0; j<4; j++){
    ...
    for( int k = 0; k<32; k++){
        weights2[k] = weights[(int)k/8][k%8];
       // printf("%f",weights[(int)k/8][k%8]);
    }
...
#define CREATE_GROUND_COLLISION_OBJECT 1
    ...
    CreateCylinder(6, -3.0, 1., .6, .2, 1., 0., 0., 0.);      //xright vertical
...
void RagdollDemo::clientMoveAndDisplay()
{
    ...
    if(timeStep == 1000)
    {
        ...
        //printf("%f %f %f\n", pos.x(), pos.y(), pos.z());
        ofstream myfile;
        myfile.open ("/Users/bmcomber/Assignment10.2/fits2.dat");
        myfile << pos.z();
        myfile.close();
        exit(0);
    }
...
/*for (int i=0; i<8; i++)
 ...
 for (int j=0; j<4; j++)
 {
 motorCommand = motorCommand + touches[(j+5)]*weights[i][j];
 }
 motorCommand = tanh(motorCommand);
 motorCommand = motorCommand*M_PI_4;
 ActuateJoint(i,motorCommand,M_PI_2);
 }
 */
